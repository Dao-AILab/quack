...
# The counter layout is a documented contract, so the host model below spells
# it with literals rather than importing it: 28 offset bits, 4 batch bits.
# Stream offsets probing the widened field: the thread index alone, an offset
# past the old 16-bit field (aliased into the batch index before), and the
# top of the 28-bit field.
OFFSET_BASES = [0, 1 << 16, (1 << 28) - N_THREADS]


def test_sr_counter_layout_contract():
    assert SR_OFFSET_BITS == 28
    assert SR_MAX_GROUPS == 16
...
# One 16-bit element past the per-call limit (SR_MAX_GROUPS batches of 4 pairs).
N_OVER_LIMIT = SR_MAX_GROUPS * 8 + 8


@cute.kernel
def _frag_kernel_bf16_over_limit(mX: cute.Tensor, mOut: cute.Tensor, seed: Int32):
    tidx, _, _ = cute.arch.thread_idx()
    frag = cute.make_rmem_tensor(cute.make_layout(N_OVER_LIMIT), Float32)
    for j in cutlass.range_constexpr(N_OVER_LIMIT):
        frag[j] = mX[tidx * N_OVER_LIMIT + j]
    out = convert_f32_frag_sr(frag, cutlass.BFloat16, seed, tidx)
    out_i32 = cute.recast_tensor(out, Int32)
    for j in cutlass.range_constexpr(N_OVER_LIMIT // 2):
        mOut[tidx * (N_OVER_LIMIT // 2) + j] = out_i32[j]


@cute.jit
def _frag_launch_bf16_over_limit(mX: cute.Tensor, mOut: cute.Tensor, seed: Int32):
    _frag_kernel_bf16_over_limit(mX, mOut, seed).launch(grid=(1, 1, 1), block=(1, 1, 1))
...
@pytest.mark.parametrize("offset_base", OFFSET_BASES)
@pytest.mark.parametrize("seed", [0, 12345])
def test_bf16_frag_sr_philox_wiring(monkeypatch, seed, offset_base, hw):
...
def test_frag_sr_rejects_more_than_max_groups():
    """A fragment needing more Philox batches than the 4-bit batch-index field
    holds (> SR_MAX_GROUPS * 8 16-bit elements) must fail at trace time, not
    alias its extra batches into the caller's offset bits."""
    x = torch.zeros(N_OVER_LIMIT, dtype=torch.float32, device="cuda")
    out = torch.empty(N_OVER_LIMIT // 2, dtype=torch.int32, device="cuda")
    with pytest.raises(Exception, match="at most"):
        cute.compile(_frag_launch_bf16_over_limit, from_dlpack(x), from_dlpack(out), Int32(0))

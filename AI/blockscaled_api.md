...
| D9 | **torch.compile support via pytree registration** plus an `allow_in_graph` construction helper for in-graph quantize. `_sf_encode` (e8m0->uint8 view across the op boundary, an Inductor `decompose_auto_functionalized` workaround) sits at the unwrap site. |
...
- Constructors: `from_parts(qdata, scale, format, *, per_tensor_scale=None,
  ...
  `allow_in_graph` helper (in-graph construction under torch.compile needs it).
  Explicit `dequantize()`; never implicit.

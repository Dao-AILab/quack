...
def _convert_f32_sr_impl(src_vec, seed, tid, cvt_fn, dst_numeric, loc, ip):
    ...
    # No num_pairs % 4 requirement: one Philox call yields 4 words and each
    # bf16/f16 pair consumes one, so a partial batch pays ~a full 7-round
    # Philox for its unused words (the rounds are one dep chain — DCE can
    # only trim the last round's dead extractions). That entropy economy is
    # accepted: small-fragment callers exist (e.g. 4-element fragments when
    # a 128-bit vectorized copy is sized by an fp32 operand). NB if fp8 is
    # ever wired in here, its unit is the QUAD, as a hard requirement: the
    # hw instruction is f8x4-only and one rand word covers exactly 4 values
    # (8 rbits each), with sw pairs carving bytes 0-1 / 2-3 of the SAME word
    # for parity — so fp8 would assert num_elems % 4 == 0 (hard) and % 16
    # for full-batch economy.

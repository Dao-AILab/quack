...
def _split_k_reduce_out(
    ...
    def scalar_arg(mode, imm, tens):
        if mode == 0:
            ...
            # Under compile-only the launch is a no-op and tens is a FakeTensor whose
            # data_ptr() is deprecated (slated to raise) — any pointer value works.
            return tens.data_ptr() if not is_compile_only() else 0
...
def split_k_reduce(
    ...
    # --compile-only / FakeTensorMode and populates the .o cache (the launch itself is a
    # no-op there since jit_cache returns _noop_kernel).
    _split_k_reduce_out(
        ws,
        out,
        split_k,
